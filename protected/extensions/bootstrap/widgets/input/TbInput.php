...
abstract class TbInput extends CInputWidget {
    ...
    /**
     * @var string append type, button or span
     */
    public $appendType;

    /**
     * @var string prepend type, button or span
     */
    public $prependType;
    ...
    protected function processHtmlOptions() {
        if (isset($this->htmlOptions['prepend'])) {
            ...
            $this->prependType = $this->htmlOptions['prependType'];
            unset($this->htmlOptions['prepend']);
            unset($this->htmlOptions['prependType']);
        }

        if (isset($this->htmlOptions['append'])) {
            $this->appendText = $this->htmlOptions['append'];
            $this->appendType = $this->htmlOptions['appendType'];
            unset($this->htmlOptions['append']);
            unset($this->htmlOptions['appendType']);
        }
    ...
    protected function getAppend() {
        if ($this->hasAddOn()) {
            ...
            if (isset($this->appendText)) {
                if($this->appendType == 'button') {
                    echo CHtml::tag('button', $htmlOptions, $this->appendText);
                } else {
     ...
     * ### .iconselectorField()
     * 
     * Renders a spinedit field.
     *
     * @return string the rendered content
     * @abstract
     */
    abstract protected function spineditField();
}
