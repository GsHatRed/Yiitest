...
class Bootstrap extends CApplicationComponent {
    ...
    public $tooltipTemplate = '<div class="tooltip"><div class="tooltip-shadow"><div class="tooltip-container"><div class="tooltip-arrow"><em></em><span></span></div><div class="tooltip-inner"></div></div></div></div>';
    ...
    /**
     *
     * @var bool 是否使用modal prompts
     */
    public $enablePrompts = true;
    
    /**
     *
     * @var type 是否启用增强modal
     */
    public $enableModalManager = true;
    ...
    public function registerJS($position = CClientScript::POS_HEAD) {
        ...
        if($this->enablePrompts) {
            $cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap-prompts.js', $position);
        }
        
        $cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap' . (!YII_DEBUG ? '.min' : '') . '.js', $position);
        
        if($this->enableModalManager) {
            $cs->registerCssFile($this->getAssetsUrl() . '/css/bootstrap-modal.css');
            $cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap-modal.js', $position);
            $cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap-modalmanager.js', $position);
        }
    }
